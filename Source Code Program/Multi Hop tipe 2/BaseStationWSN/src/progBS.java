...
public class progBS {
	...
	int[] nodeAddress = {0xBABE, 0xBEBA, 0xBAFE, 0xAFFE};
	int[] NODE_ADDRi  = {47806,  48826,  47870,  45054};
	...
	public void recvReply(final FrameIO fio, final long t1) throws Exception {
		new Thread() {
			public void run() {
				...
				int count = 0; 
				int snSebelumnya = -1;
				while ((count < bcTableStatus.length) && ((Time.currentTimeMillis() - t1) <= 6000)) {
					try {
	...
	public void sendACK(long originalSrcAddr, int ackSN) throws Exception {
	    try {
	        Frame frame = new Frame(Frame.TYPE_DATA |
	                Frame.DST_ADDR_16 | Frame.INTRA_PAN | Frame.SRC_ADDR_16);
	        frame.setSrcAddr(myAddress);
	        frame.setSrcPanId(panID);
	        frame.setDestAddr(nodeAddress[1]);
	        frame.setDestPanId(panID);
	        frame.setSequenceNumber(sequenceNumber++);
	        String message = "110 " + Long.toHexString(originalSrcAddr) + " " + ackSN;
	        frame.setPayload(message.getBytes());
	        radio.setState(AT86RF231.STATE_TX_ARET_ON);
	        fio.transmit(frame);
	        System.out.println("ACK dikirim untuk " + Long.toHexString(originalSrcAddr) + " SN=" + ackSN);
	    } catch (Exception e) {
	        e.printStackTrace();
	    }
	}
	...
	public void recvSense(final FrameIO fio) {
	    new Thread() {
	        public void run() {
	            ...
	            //sn terakhir yang di-ACK
	            final Map<Long, Integer> lastAckedSN = new HashMap<>();
	            ...
	            while (!stop) {
	                ...
	                //Original berdasarkan relay 
	                long curSrcAddr = frame.getSrcAddr();

	                byte[] payloadBytes = frame.getPayload();
	                String pesanDiterima = new String(payloadBytes, 0, payloadBytes.length);

	                String[] parts = StringUtils.split(pesanDiterima, " ");
	                if (parts.length < 2) {
	                	continue;
	                }
	                ...
	                int snSenseParsed = -1;
	                try {
	                    snSenseParsed = Integer.parseInt(parts[1]);
	                } catch (NumberFormatException e) {
	                    continue;
	                }

	                // Cek apakah ini data baru atau duplikat snSense
	                Integer lastAck = lastAckedSN.get(curSrcAddr);
	                boolean isNew = false;

	                try {
	                    if (lastAck == null) {
	                        if (snSenseParsed == 0) {
	                            lastAckedSN.put(curSrcAddr, 0);
	                            isNew = true;
	                            sendACK(curSrcAddr, 0);
	                        }
	                    } else if (snSenseParsed == lastAck + 1) {
	                        lastAckedSN.put(curSrcAddr, snSenseParsed);
	                        isNew = true;
	                        sendACK(curSrcAddr, snSenseParsed);
	                    } else if (snSenseParsed <= lastAck) {
	                        isNew = false;
	                        sendACK(curSrcAddr, lastAck);
	                    }
	                } catch (Exception e) {
	                    e.printStackTrace();
	                }

	                // Tulis ke USART hanya kalau data baru
	                if (isNew) {
	                    lock.lock();
	                    try {
	                        String toPC = "#" + pesanDiterima + "\n#";
	...
	public void cekNodeAktif() throws Exception {
		...
		sendTONODE(fio, message, sequenceNumber++, nodeAddress[1]);
		recvReply(fio, t1);
	}
	...
	public void sinkronisasiWaktu(int hi) throws Exception {
	    ...
	    // "010 <deltaBEBA> <deltaBAFE> <deltaAFFE>"
	    String message = "010"
	        + " " + rttTable[1] / 2   // untuk BEBA
	        + " " + rttTable[2] / 2   // untuk BAFE
	        + " " + rttTable[3] / 2;  // untuk AFFE
	    sendTONODE(fio, message, sequenceNumber++, nodeAddress[1]); // ke BEBA
	    recvReply(fio, t1);
	}
