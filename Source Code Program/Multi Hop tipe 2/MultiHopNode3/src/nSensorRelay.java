...
//Kelas ini digunakan sebagai relay pada multi hop tipe 2 
public class nSensorRelay {
    ...
    //Alamat node saat ini yaitu BEBA
    int indexNode = 3;
    int alamatNode = nodeAddress[indexNode];

    //NextHop nya adalah BABE
    int nextHop = nodeAddress[0];
    
    //Mempunyai 2 previous hop yaitu BAFE, dan AFFE
    int[] prevHops = {nodeAddress[indexNode-1], nodeAddress[indexNode-2]};
    ...
    public void cekNodeAktif(String[] split, long t2) throws Exception {
        ...
        sendFrameToBS(msg, sn++, alamatNode);
    }
    ...
    private boolean isDariPrevHop(long srcAddr) {
        for (int ph : prevHops) {
            if (srcAddr == ph) return true;
        }
        return false;
    }
        ...
        new Thread() {
            ...
            public void run() {
                ...
                try {
                    ...
                    long srcAddr = f.getSrcAddr();
                    boolean dariArahBS = (srcAddr == nextHop);

                    if (!dariArahBS) {
                        if (isDariPrevHop(srcAddr)) {
                            relayToNextHop(pesanDiterima, sn++, srcAddr);
                        }
                        return;
                    }
                    ...
                    if (code >= 1 && code <= 4 || code == 6) {
                        for (int ph : prevHops) {
                            relayToPrevHop(pesanDiterima, sn++, ph);
                        }
                    }

                    switch (code) {
                        case 1: {
                            cekNodeAktif(splitPesan, t2);
                            break;
                        }
                        case 2: {
                            sinkronisasiWaktu(splitPesan, t2);
                            break;
                        }
                        case 3: {
                            kirimkanWaktu(splitPesan, t2);
                            break;
                        }
                        case 4: {
                            nextSeqNum = 0;
                        ...
                        case 5: {
                            long targetAddrLong = Long.parseLong(splitPesan[1], 16);
                            // relay ACK ke prevHop yang sesuai
                            for (int ph : prevHops) {
                                relayToPrevHop(pesanDiterima, sn++, ph);
                            }
    ...
    private void sendSenseToNextHop(final String msg, final int snVal) throws InterruptedException {
        ...
        while (!ok) {
            try {
                Frame frame = new Frame(Frame.TYPE_DATA | Frame.ACK_REQUEST | Frame.DST_ADDR_16 | Frame.INTRA_PAN | Frame.SRC_ADDR_16);
    ...
    public void relayToPrevHop(final String msg, final int snVal, final int destAddr) {
        new Thread() {
            public void run() {
                boolean ok = false;
                while (!ok) {
                    try {
                        ...
                        System.out.println("relay to " + Integer.toHexString(destAddr) + ":" + msg);
                        ok = true;
                    } catch (RadioDriverException e) {
                    } catch (NoAckException e) {
                    } catch (ChannelBusyException e) {
    ...
    private void sendFrameToBS(final String msg, final int snVal, final long srcAddr) {
        new Thread() {
            public void run() {
                boolean ok = false;
                while (!ok) {
                    ...
                    try {
                        ...
                        if (!buffer.isEmpty() && (now - startTimer > timeout)) {
                            ...
                            while (seq < nextSeqNum) {
                                ...
                                if (data != null) {
                                    sendSenseToNextHop("SENSE " + data, seq);
                                }
                                seq++;
                            }
                            startTimer = now;
