...
//Kelas ini merupakan node paling ujung dalam topologi multi hop 
public class nSensorMulti {
    ...
    //Untuk AFFE index node 1
    //Untuk BAFE index node 2 
    int indexNode = 1;
    int alamatNode = nodeAddress[indexNode];

    //Jika toplogi MH tipe 1, maka NextHop nya adalah AFFE 
    
    //Jika topologi MH tipe 2
    //Jika AFFE, maka nextHop BEBA 
    //Jika BAFE, maka nextHop BAFE
    int nextHop = nodeAddress[indexNode + 2];
    ...
    private void sendReplyToNextHop(final String msg, final int snVal) {
        new Thread() {
            public void run() {
                boolean ok = false;
                while (!ok) {
